...
export default function ChannelDetailPage({ params }: { params: { channelId: string } }) {
    ...
    React.useEffect(() => {
        const fetchChannel = async () => {
            ...
            try {
                const fetchedChannel = await getChannelById(params.channelId, session.token);
                if (!fetchedChannel) {
                    notFound();
                } else {
        ...
        fetchChannel();
    }, [params.channelId, toast]);

    if (channel === undefined) {
        return (
             <div className="flex-1 space-y-4 pt-6">
                <div className="flex items-center justify-center h-[450px]">
                    <div className="text-2xl font-bold tracking-tight">Loading channel data...</div>
                </div>
            </div>
        )
    }
            ...
            <div className="flex-1 space-y-4 pt-6">
                 <div className="flex flex-col items-center justify-center h-[450px]">
                    ...
                    <p className="text-sm text-muted-foreground mb-4">
                        There was an error fetching the channel data. Please try again later.
                    </p>
                    <Link href="/channel" className="text-sm text-muted-foreground hover:text-foreground">
